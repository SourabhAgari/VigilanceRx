...
| 11 | Logging & Observability | cloud | ◐ in progress — started 2026-08-10. Epic #145 + child issues #146–#153 created; scope expanded beyond spec minimum (D66). #146–#151 done, plus the #175 fix; #152–#153 remain, and #152 needs the cluster for its stall drill. #178 (Argo CD / External Secrets metrics) was raised out of #151 and is not a blocker |
...
- [x] #150 New application metrics (`deadLetterRecords`,
      ...
      **`broadcastEntriesLoaded` also reads 0, and that does not mean the
      ...
      — **resolved 2026-08-15 (D75)**: `broadcastEntriesLoaded` is deleted. The
      contract above changes in one place — it exists on neither operator, and
      `chronic_class_filter` publishes `bufferedFillsAwaitingRef` instead;
      `adherence_process` has no broadcast gauge at all
- [x] #175 `broadcastEntriesLoaded` replaced by `bufferedFillsAwaitingRef`
      — done 2026-08-15: 151 unit + 11 integration tests green
      (`mvn clean verify`). Found while capturing the metric contract above,
      and fixed before #151 could build a panel on it or #152 an alert. See
      **D75**

**Cloud — billing starts here**
- [x] #151 Four Grafana dashboards (support, developer, platform, business),
      provisioned as ConfigMaps so they survive a Grafana pod restart
      — done 2026-08-15: 54 panels across the four, every one carrying a
      description saying what it means and what to do when it is wrong — the
      difference D66 named between *monitorable* and *supportable*.
      **Provisioning proven the way the issue asked for**: the Grafana pod was
      deleted and all panels came back, which a dashboard clicked together in
      the UI does not survive. **Location revised — dashboards live in
      `k8s/flink/grafana-dashboards/` in the `rx-vigilance` namespace, not
      `observability/` in `monitoring`; see D76.**
      **Every metric name was read off the cluster before a panel was written**,
      never inferred, after that mistake had already cost time twice (#146's
      operator labels, D73's RocksDB names). Two findings came out of it. The
      RocksDB metrics carry `column_family` as a scope segment *inside* the
      name — `flink_taskmanager_job_task_operator_column_family_rocksdb_*` — so
      the obvious query returns nothing. And `broadcastEntriesLoaded` was still
      listed in Prometheus after #175 deleted it: a series inside the retention
      window that nothing emits any more, which would have drawn a panel that
      worked on the day it was built and flatlined the next.
      **Shared design system across all four** (D77): Apple's system palette
      with one colour per meaning, transparent panels, smooth 2px lines with a
      12% gradient, shared crosshair, large numerals. Status tiles on Support
      and Platform keep filled backgrounds deliberately — that row is read by
      someone who has just been paged.
      **Two panels the issue asked for could not be built.** "Alerts by drug
      class" needs a label no counter carries, and #150's naming rules keep
      identifiers out of labels on purpose. "Lag from fill date to alert" is not
      measured at all; `currentEmitEventTimeLag` exists only on sources and
      reads -1. Both need code, both are filed separately, and watermark lag
      stands in as the honest freshness signal meanwhile.
      **Argo CD and External Secrets panels are deferred to #178** — measured
      2026-08-15, Prometheus holds zero metrics for either, because neither
      Helm release exposes them. Everything else the Platform dashboard needs is
      covered by the 159 kube-state and 266 node metrics already scraped.
      **For #152**: watermark lag grows without bound whenever a source is idle
      — it read 1.62 days on a perfectly healthy job. A naive `lag > 1h` rule
      would fire permanently here. `sourceIdleTime` is what separates *idle*
      from *stuck*, and the two are paired on the Developer dashboard for
      exactly that reason
- [ ] #152 `alertmanager-rules.yaml`: seven critical + eight warning rules,
      ...
      — **⏸ deferred 2026-08-16 by decision, not by blocker (D78)**. Log
      viewing in Grafana was judged the more useful next thing, so #181 (the
      Cloud Logging datasource) and #182 (the two log dashboards) go first.
      **Consequence stated plainly: exit criteria 6 and 7 are #152's, so
      Phase 11 cannot close until it is done.** It stays inside Phase 11 rather
      than moving to Phase 12 — the alerts belong with the observability work
      that gives them something to point at
- [x] #181 Grafana Cloud Logging datasource — the prerequisite nobody had
      noticed: Grafana holds only Prometheus and Alertmanager datasources, so
      it cannot read a single log line. Plugin, a `logging.viewer` service
      account under Workload Identity, and the datasource provisioned from git
      — done 2026-08-16: `googlecloud-logging-datasource` v1.7.0 installed,
      datasource `gcp-cloud-logging` registered, and its health check returned
      `{"message":"Successfully queried logs from GCP project
      vigilancerx-502702","status":"OK"}` — which is the evidence that matters,
      because it exercises the whole chain (pod → Workload Identity →
      `rx-vigilance-grafana` → `roles/logging.viewer` → real log data) rather
      than merely proving the plugin unpacked. No key file anywhere; nothing
      secret entered Terraform state. Provisioning location decided as D80
- [ ] #182 Application and Infrastructure log dashboards — two views over the
      one datasource. Separation by view and permission, never by store: during
      an incident the app error and the node problem must be readable side by
      side (D79)
...
| D74 | 2026-08-14 | **`spot = false` on the GKE node pool is retained, deviating from D7, for the duration of the Phase 11 observability work (cluster up 2026-08-14 → 2026-08-18).** The comment on the line is corrected so the file stops contradicting itself | D7 chose spot nodes at ~70% saving on the reasoning that preemption is fine on a self-healing demo, and for Phases 1–10 it was. Phase 11 is the exception: #151–#153 are about watching metric and log behaviour over time, and a preemption restarts the job, which resets every per-subtask counter and gauge to zero. #175 is the concrete illustration — `broadcastEntriesLoaded` reads 0 after a restart whether or not anything is wrong — so a preemption mid-session produces a dashboard full of zeros that looks like a genuine failure and costs an investigation to disprove. Paying non-spot rates for two days to avoid that is the cheaper trade. **This is time-boxed, not a reversal of D7.** The value returns to `true` when the runtime stack is destroyed on 2026-08-18, and D7 stands for every phase after. Recorded because the change reached `main` inside PR #174 with no issue, no PR mention and a comment that still described the old value, which is exactly the drift §2 exists to prevent — the line now reads `spot = false # D74: non-spot for the Phase 11 observability window; revert to true per D7` |
| D75 | 2026-08-15 | **`broadcastEntriesLoaded` is deleted and replaced by `bufferedFillsAwaitingRef` on `ChronicClassFilterFunction` only** (#175). The lead-time broadcast gets no replacement gauge | **The defect.** The gauge counted entries arriving through `processBroadcastElement` since `open()`. A restore repopulates broadcast state from the checkpoint without a single such call, so the gauge read 0 on a healthy restart — indistinguishable from the genuinely empty broadcast it was added to expose (#150's stated premise). Found on the cluster 2026-08-14, where it read 0 on both operators while `committedOffset = 1` on both reference topics proved the data was loaded. An alert on `== 0` would have fired after every restart, which is how alerts get muted and then ignored. **Why measuring the buffer rather than the broadcast.** Two candidate fixes were rejected. Persisting the arrival count in operator state survives restore but counts updates as well as distinct keys, so on a compacted ref topic the number drifts above the truth, and it needs new operator state on both operators. Counting broadcast state on the first `processElement` is accurate but only runs when records flow — and "nothing is flowing" is precisely the situation the metric exists for. The buffer avoids both: `ChronicClassFilterFunction` already parks every fill whose drug-class ref is missing, that buffer is operator `ListState`, and unlike broadcast state it **is** readable in `initializeState`, so the count is seeded from the restored buffer before a single record moves. It also measures the harm rather than the input — "40,000 fills are stuck" tells an on-call what to do; "the broadcast holds 12 entries" does not. **The ordering that makes it correct, and that is worth remembering.** `initializeState` runs *before* `open()`. The counter is created and seeded there, and `open()` registers the gauge without assigning the field — assigning it in `open()` would reset the restored count to zero, the identical shape of the bug being fixed. `bufferedFillsGaugeIsCorrectAfterARestoreWithNothingFlowing` snapshots a harness holding two buffered fills, discards the operator, restores into a new one and asserts the gauge reads 2 with no record processed, then drains the buffer with the ref to prove the number and the contents agree. **Why `adherence_process` loses its gauge outright.** An absent lead-time ref parks nothing — it silently falls back to the configured default, and `missingLeadTimeLookup` already counts every such fallback. A second metric for the same failure would be two numbers that can disagree. **Known limitation, stated rather than glossed:** if no fills arrive at all, an empty drug-class broadcast stays invisible. Acceptable because an empty broadcast harms nothing while nothing is flowing, and "no fills arriving" is separately visible via `numRecordsIn` and `sourceIdleTime`. **Cost:** #150's frozen name list changes, which is only cheap because no dashboard exists yet — the last moment this is free |
| D76 | 2026-08-15 | **Grafana dashboards ship as ConfigMaps in `k8s/flink/grafana-dashboards/`, created in the `rx-vigilance` namespace and synced by Argo CD — not in `observability/` applied by hand into `monitoring`. Revises D68.** The Argo CD `AppProject` gains an eighth permitted kind, `ConfigMap` | **What changed since D68.** D68 reserved `observability/` for #151 and #152 on the reasoning that dashboards configure the monitoring stack, which lives in `monitoring`, a namespace the Application has no business touching. That was sound given what was known, and one measured fact undoes it: the Grafana dashboard sidecar runs with `NAMESPACE=ALL`. Read off the live pod on 2026-08-15 alongside `LABEL=grafana_dashboard` and `LABEL_VALUE=1`. It watches every namespace, so a ConfigMap in `rx-vigilance` is picked up exactly like one in `monitoring`. That is not a workaround — `NAMESPACE=ALL` exists so an application team can ship dashboards from its own namespace. **Why that decides it.** Argo CD reads `path: k8s/flink` and nothing else (D54). A dashboard in `observability/` would be valid YAML, committed to `main`, and never deployed, with nothing anywhere reporting a problem — the identical failure D68 itself refused for `podmonitor.yaml`, and the precedent is already set: PodMonitor is monitoring configuration and lives in `k8s/flink` for this reason. Inside Argo's path the dashboards are deployed on merge, restored if deleted, and survive a cluster rebuild with no remembered manual step. That last point is concrete rather than theoretical: this cluster is scheduled for `terraform destroy` (D74), and a `kubectl apply` nobody writes down is a step nobody repeats. **What it costs.** The AppProject whitelist gains `ConfigMap`, which widens Argo's reach in `rx-vigilance`. Applied by hand before the first ConfigMap reached `main`, because the AppProject is deliberately not Argo-managed and D68 records what happens otherwise — the sync fails on an unpermitted kind, several steps from the cause. Prune is safe because Argo only prunes resources carrying its own tracking label, which the Flink operator's own ConfigMaps never have; verified after the first sync the same way D54 verified the hand-made Secrets. **The split D68 drew still holds, just redrawn.** `k8s/flink/` is everything deployed into the job's namespace, whatever it configures. `observability/` remains right for #152's alert rules, which are `PrometheusRule` objects consumed by Alertmanager in `monitoring` and have no equivalent escape hatch. **One thing that does not work and was removed.** The `grafana_folder` annotation is inert here — the sidecar has no `FOLDER_ANNOTATION` env var, so dashboards land in Grafana's default location regardless. Enabling it means a `helm.tf` values change and a Helm upgrade for a cosmetic gain; the shared `rx-vigilance` tag groups them adequately and also drives the cross-dashboard drill-down dropdown |
| D77 | 2026-08-15 | **The four dashboards share one visual system, applied by script rather than panel by panel** | A dashboard is read under pressure by someone who did not build it, so the design is a correctness concern, not decoration. **One colour vocabulary.** Apple's system palette, assigned by meaning and identical across all four: green is prevention working, orange is something that slipped, red is data lost, blue is throughput, grey is expected and uninteresting. Gap-risk alerts are the same green on Support, Developer and Business, so it is learned once rather than three times. **Applied by script for a reason.** Fifty-four panels styled by hand drift; a script that walks the JSON cannot. The same script also normalised every Grafana named colour to its hex equivalent, which caught two tiles that had been missed. **Chrome removed** — `transparent: true` on every panel, smooth 2px lines with a 12% gradient and no point markers, axis borders off. **Shared crosshair** (`graphTooltip: 1`) so hovering one panel marks the same instant on all of them; that single setting is what makes a page read as one instrument rather than twelve. **One deliberate inconsistency, recorded so it is not 'fixed' later.** Status tiles on Support and Platform keep filled background colour while everything else uses coloured numerals on plain. Those rows are read at a glance by someone who has just been paged, possibly from a wall display, and legibility beats restraint there — the same reason Apple uses solid red badges and Activity rings for status while keeping Health's data views quiet |
| D78 | 2026-08-16 | **#152 (alert rules) is deferred inside Phase 11, and log viewing in Grafana takes its place as the next work.** Phase 11 stays open until #152 is done | **The choice.** With the cluster up until 2026-08-18 the question was what to spend it on. Alerts were next in the plan; log viewing was judged more useful first, on the reasoning that an alert tells you something is wrong and a log tells you what — and #151 has already made "something is wrong" visible on four dashboards. **The consequence, stated rather than discovered later.** Phase 11 has eleven exit criteria and two of them belong to #152: alert rules passing `promtool test rules` in CI, and the induced watermark-stall drill. Deferring #152 therefore defers the close of Phase 11. It is a deliberate reordering, not a quiet drop. **Why it stays in Phase 11 rather than moving to Phase 12.** The alerts exist to point at the dashboards, runbook sections and log queries this phase is building. Moved to Phase 12 they would be written against work that has aged, and Phase 12's soak is the natural place to *validate* thresholds rather than invent them. **A prerequisite it inherits either way.** Two records have been through this job in its life. Thresholds like "consumer lag growing" and "sustained backpressure" have no baseline to be set against, and a threshold chosen without ever seeing normal is a guess. The critical rules do not have this problem — "job down" and "checkpoints failing" are absolutes — so #152 can be split when it resumes: absolutes now, rate-based rules after a sustained producer exists |
| D79 | 2026-08-16 | **Application and infrastructure logs are separated by view and by permission, never by store.** One pipeline, one destination, two dashboards | **Why not separate stores.** The obvious move is to route application logs somewhere and infrastructure logs somewhere else. It is the wrong one for the case that matters: during an incident the question is "what happened at 14:03", and the answer is routinely an application error and a node problem three seconds apart. Split across two stores that becomes two tools and timestamp alignment by eye, at the moment that is least affordable. **What separation is actually for.** Two things, and both are satisfied without splitting storage. *Views*: an Application dashboard and an Infrastructure dashboard over the same datasource, which is a filter rather than an architecture — the logs already carry `jsonPayload.service="rx-vigilance"` from #147 and `resource.labels.namespace_name` from GKE, so nothing needs building to distinguish them. *Permission and retention*: this is the one with a real justification here, because §9 says treat member data as if it were PHI and D71 puts `memberId` in application logs at DEBUG, while infrastructure logs never carry it. Different readers and different retention for those two is defensible; different buckets for tidiness is not. **What is deliberately not being done yet.** The retention and access split needs Cloud Logging log buckets and a log router in Terraform. Worth doing on §9 grounds and filed separately rather than bolted onto the dashboard work, because it is an IAM change and deserves its own plan. **The practical consequence for #182.** Two dashboards, one datasource, and they must be linkable to each other with the time range preserved — the same cross-dashboard dropdown #151 already uses, for the same reason |
| D80 | 2026-08-16 | **The Cloud Logging datasource is provisioned in the kube-prometheus-stack Helm values (`infra/terraform/runtime/helm.tf`), not as a labelled ConfigMap under `k8s/flink/` like the dashboards.** Its `uid` is pinned to `gcp-cloud-logging` | **Why the D76 pattern does not carry over.** D76 put dashboards in `k8s/flink/` because the dashboards sidecar runs `NAMESPACE=ALL`, so a ConfigMap in `rx-vigilance` is picked up and Argo CD deploys it on merge. The **datasources** sidecar in the same pod has no `NAMESPACE` set at all, so it defaults to its own namespace and watches `monitoring` only — verified on the running deployment before deciding, not assumed. A datasource ConfigMap under `k8s/flink` would have been synced by Argo CD into `rx-vigilance` and then silently ignored: present in git, absent from Grafana, with nothing logging a complaint. That failure mode is the same class as D69 and is exactly why the check came first. **Why Helm values rather than adding `searchNamespace: ALL` to the second sidecar.** Widening the sidecar would have made the ConfigMap work, but the datasource is platform wiring, not application config — it pairs with the plugin install and the Workload Identity binding in `platform/grafana.tf`, and all three now move together in one `terraform apply`. Dashboards stay app-owned because they change when application metrics change; Grafana's own plumbing stays Terraform-owned because it changes when the platform does. **Why the uid is pinned.** The #182 log dashboards reference the datasource by uid. Left to Grafana it would be a generated string, and the dashboard JSON in git would either hardcode a value that differs per environment or need a datasource template variable to paper over it. **On credentials.** `authenticationType: gce` reads the identity from the metadata server, which is how Workload Identity delivers it — so no key file exists to leak and nothing secret enters Terraform state, satisfying §9 by construction rather than by care |
