...
- [x] #150 New application metrics (`deadLetterRecords`,
      ...
      **Metric-name contract, read off the live `/metrics` endpoint on
      2026-08-14** (TaskManager `rx-vigilance-taskmanager-1-1`), not assumed.
      Application metrics all carry the prefix
      `flink_taskmanager_job_task_operator_adherence_` and are separated by the
      `operator_name` label:
      ...
      `bufferedFillsAwaitingRef`;
      `{rx_fill_events,ndc_drug_class_ref,alert_lead_time_ref}_dead_letter_split`
      — `deadLetterRecords`, three series, one per source, as intended.
      (Underscored topic and operator names are Prometheus label sanitisation;
      the real topics are hyphenated as in `redpanda.tf`.)
      **Lazy registration is confirmed working**: `chronic_class_filter`
      publishes only its own two metrics, with no permanently-zero alert
      counters alongside them.
      **The RocksDB names are not what the config keys suggest** — #146's
      lesson repeating. `column_family` appears both as a scope segment *in the
      name* and as a label:
      ...
      **State of the job at capture time, which #151 must account for**: every
      ...
      separate two facts that are easy to conflate: both reference topics show
      `committedOffset = 1`, so the drug-class and lead-time data **was**
      consumed and is held in state, while `rx-fill-events` shows
      ...
      **`broadcastEntriesLoaded` also read 0, and that did not mean the
      broadcast was empty — the gauge was defective. Filed as #175 and since
      replaced; see D75.** It counted entries *arriving* via
      `processBroadcastElement` since `open()`, and a restore repopulates
      broadcast state without any such call, so it read 0 on a healthy restart
      exactly as it would on a genuinely empty broadcast — the one distinction
      #150 added it to make. An earlier draft of this note read the 0 as "the
      reference data never loaded"; that was wrong and is corrected here. The
      contract above already reflects the replacement: `broadcastEntriesLoaded`
      exists on neither operator, `bufferedFillsAwaitingRef` is on
      `chronic_class_filter` only, and `adherence_process` has no broadcast
      gauge at all
- [x] #175 `broadcastEntriesLoaded` replaced by `bufferedFillsAwaitingRef`
      — done 2026-08-15: 151 unit + 11 integration tests green
      (`mvn clean verify`). Defect found while capturing the metric contract
      above, fixed before #151 could build a panel on it or #152 an alert. See
      **D75**
