...
- [x] #150 New application metrics (`deadLetterRecords`,
      ...
      **Not verified, and must not be treated as verified**: the RocksDB
      metric *names*. Every test in this repo runs on `HashMapStateBackend`
      ("No state backend has been configured, using default (HashMap)" in the
      test output), so nothing local exercises RocksDB. What is proven locally
      is that the four keys land under `spec.flinkConfiguration` as strings —
      checked by parsing `kubectl kustomize k8s/flink`, not grepping it (D69).
      #151 must read the real strings off `curl localhost:9249/metrics` the
      way #146 did before building any panel on them
