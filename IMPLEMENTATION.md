...
- [x] #105 Verify job healthy on GKE against cloud Redpanda through one
      ...
      — done 2026-08-09: checkpoints 50→81 completed, every 30s, ~33.5 kB,
      ~1s duration; JM and TM pods 0 restarts; `RUNNING`/`STABLE`. Proves
      the transactional sink commits, Workload Identity reaches the bucket,
      the GCS plugin loads, and `state.checkpoints.dir` is correct. Full
      loop exercised: merge → Actions build → GHCR → App-token commit →
      Argo CD sync → operator savepoint upgrade → running job
- [ ] #132 Single-source the checkpoint directory — the `gs://` path is written
...
- [ ] #133 Revisit `transaction.timeout.ms` — 60000 was kept because the alert
...
- [ ] #118 Replace the hand-created Secrets with External Secrets Operator +
...
- [ ] #115 Set resource requests on cert-manager, the Flink operator and
