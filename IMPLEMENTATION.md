...
| 9 | Metrics, job wiring & integration test | local | ✅ done 2026-08-03 (IT hardened + topology test added 2026-08-05 — D41, D42) |
...
- [x] `AdherenceMetricsReporter`: alert emission counters, filter drop rate,
      ...
      — gap found 2026-08-05: all four counters are **wired but untested** —
      no test asserts their registered names or that they increment. This
      task's evidence above covers wiring only. Phase 11 dashboards will key
      on these exact names, so a typo surfaces as an empty panel in a [CLOUD]
      phase. Deferred to #92 rather than expanded into this PR: asserting
      metric registration needs either a new test dependency (Mockito) or
      hand-built Flink metric-group plumbing, and that choice is a Decision
      Log entry of its own. **Phase 9 should not be considered fully closed
      until #92 lands**
