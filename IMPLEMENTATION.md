...
| 10 | Containerization & CI/CD deploy path | cloud | ◐ in progress — #98, #99, #100, #109 done (D49–D52). Next: #101 node sizing, then the #30 decision gate |
...
*Infrastructure:*
- [ ] #101 Scale GKE node pool, revise D7 (credits expire ~2026-09-05).
      Captures `kubectl describe node` — the measurement that settles #30
- [ ] #102 Workload Identity Federation for GitHub Actions → GCP. **Note**:
      distinct from the GKE Workload Identity already in `gke.tf`; only the
      CI side is new work

*Pipeline:*
- [ ] #103 `.github/workflows/deploy.yml`: main-branch push → package → GHCR
      push → deploy
...
**Decision gate**: #30 (Argo CD / GitOps) is decided **after #101, before
#102** — it determines whether #102 is needed at all and what #103's final
step looks like. Deciding after Phase 10 would mean writing #103 twice and
possibly building #102 for a path that gets abandoned.
